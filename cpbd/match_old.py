...
def supermatch_matrix(lat,nrg,mag,varz):
    def error_func(x):
        ...
        err = (np.abs(lat.R[0,0])-mag[0])**2 + (np.abs(lat.R[2,2])-mag[1])**2 +\
         np.abs(10*lat.R[0,1])**2 + np.abs(10*lat.R[2,3])**2 +\
    ...
    print("initial value: x = ", x)
    fmin(error_func, x, ftol=1e-9,xtol=1e-9, maxiter=20000, maxfun=20000)
...
#         np.abs(lat.R[0,0]-mag[0])**2 + np.abs(lat.R[2,2]-mag[1])**2 #+\
#         np.abs(lat.R[0,1])**2 + np.abs(lat.R[2,3])**2 #+(np.abs(lat.T[0,1,5])-np.abs(lat.T[2,3,5]))**2
        return err
